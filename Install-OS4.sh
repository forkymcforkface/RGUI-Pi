...
check_reboot() {
    if [ -f "$flag_file" ]; then
        ...
        if [ "$current_step" -eq 0 ] || [ "$current_step" -eq 1 ] || [ "$current_step" -eq 2 ] || [ "$current_step" -eq 3 ]; then
...
step_0() {
    echo "Step 0: Adding configuration and updating system..."
    {
        ...
        echo "dtoverlay=vc4-kms-dpi-custom"
    ...
    echo "$script_dir/Install-OS4.sh" | sudo tee -a /etc/profile.d/10-rgbpi.sh > /dev/null
    {
        ...
        echo -e "ExecStart=-/sbin/agetty --skip-login --noclear --noissue --login-options \"-f pi\" %I \$TERM"
    } | sudo tee -a /etc/systemd/system/getty@tty1.service.d/autologin.conf > /dev/null
    sudo apt-get update
    echo 1 > "$flag_file"
}
